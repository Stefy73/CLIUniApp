...
class UniversityController: 
    ...
    def enroll_subject(self, subject_name):
        try:
            ...
            # Parse subject name and id
            parts = subject_name.rsplit(' ', 1)
            if len(parts) == 2 and parts[1].isdigit():
                subject_name_only = parts[0]
                subject_id = parts[1].zfill(3)
            else:
                subject_name_only = subject_name
                subject_id = str(random.randint(100, 999)).zfill(3)

            # Prevent duplicate enrollment (by name and id)
            for subj in self.current_student.subjects:
                existing_name = getattr(subj, 'name', '')
                existing_id = getattr(subj, 'id', '')
                if existing_name == subject_name_only and existing_id == subject_id:
                    messagebox.showerror("Error", f"You are already enrolled in {subject_name_only} ({subject_id})")
                    return False

            # Check total enrollment limit
            if len(self.current_student.subjects) >= 4:
                return False
            
            subject = Subject()
            subject.name = subject_name_only
            subject.id = subject_id
            subject.grade = random.choice(["HD", "D", "C", "P", "F"])
            self.current_student.subjects.append(subject)
            self.model.save_students()
            return True
        except Exception as e:
            print(f"Error enrolling subject: {e}")
            return False
...
class LoginView(tk.Frame): 
    def __init__(self, master, controller, switch_frame_callback): 
        ...
        tk.Label(self, text="Welcome to the University Portal", font=("Arial", 24), bg="darkslategrey", 
fg="white").pack(pady=20) 
        tk.Label(self, text="Please log in to continue", font=("Arial", 16), bg="darkslategrey", 
fg="white").pack(pady=10) 
 
        form_frame = tk.Frame(self, bg="darkslategrey") 
        form_frame.pack(pady=20) 
 
        tk.Label(form_frame, text="Email:", bg="darkslategrey", fg="white").grid(row=0, column=0, 
padx=10, pady=10, sticky="e") 
        self.email_entry = tk.Entry(form_frame) 
        self.email_entry.grid(row=0, column=1, padx=10, pady=10) 
 
        tk.Label(form_frame, text="Password:", bg="darkslategrey", fg="white").grid(row=1, column=0, 
padx=10, pady=10, sticky="e") 
        self.password_entry = tk.Entry(form_frame, show="*") 
        self.password_entry.grid(row=1, column=1, padx=10, pady=10) 
 
        tk.Button(form_frame, text="Login", bg="#252525", fg="#ffc107", 
command=self.login).grid(row=2, column=0, columnspan=2, pady=20) 
        tk.Label(self, text="Please note this portal is only for registered students", font=("Arial", 12), 
bg="darkslategrey", fg="white").pack(side=tk.BOTTOM, pady=10) 
...
class OptionsView(tk.Frame): 
    def __init__(self, master, controller, switch_frame_callback): 
        ...
        tk.Label(self, text="Select an Option", font=("Helvetica", 16), bg="darkslategrey", 
fg="white").pack(pady=20) 
        self.welcome_label = tk.Label(self, text="", font=("Helvetica", 16), bg="darkslategrey", 
fg="white") 
        self.welcome_label.pack(pady=10) 
 
        tk.Button(self, text="Previous Enrollments", bg="#252525", fg="#ffc107", command=lambda: 
self.switch_frame("view_subjects")).pack(pady=10) 
        tk.Button(self, text="Enroll to New Subjects", bg="#252525", fg="#ffc107", command=lambda: 
self.switch_frame("enrolment")).pack(pady=10) 
        tk.Button(self, text="Logout", bg="#252525", fg="#ffc107", 
...
fg="white") 
        ...
        tk.Label(self, text="Select up to 4 subjects to enroll:", font=("Helvetica", 14), bg="darkslategrey", 
fg="white").pack(pady=10) 
        ...
        tk.Button(button_frame, text="Submit", bg="#252525", fg="#ffc107", 
command=self.submit_enrolment).pack(side=tk.LEFT, padx=10) 
        tk.Button(button_frame, text="Back", bg="#252525", fg="#ffc107", command=lambda: 
self.switch_frame("options")).pack(side=tk.LEFT, padx=10) 
    ...
    def update_view(self): 
        ...
        for subject in available_subjects: 
            ...
            tk.Checkbutton(self.checkbox_frame, text=subject, variable=var, bg="darkslategrey", 
fg="white", selectcolor="darkslategrey").pack(anchor="w") 
...
fg="white") 
        ...
        tk.Label(self, text="Your Enrolled Subjects", font=("Helvetica", 14), bg="darkslategrey", 
fg="white").pack(pady=10) 
        ...
        tk.Button(button_frame, text="Back", bg="#252525", fg="#ffc107", command=lambda: 
self.switch_frame("options")).pack(pady=10) 
    ...
    def update_view(self): 
        ...
        if not subjects: 
            ...
            for subject in subjects: 
                ...
                tk.Label(self.subjects_frame, text=info, bg="darkslategrey", fg="white", font=("Helvetica", 
12)).pack(anchor="w", pady=3) 
...
class UniversityPortal(tk.Tk): 
    def __init__(self): 
        ...
        self.frames["view_subjects"] = ViewSubjectsFrame(self.container, self.controller, 
self.show_frame) 
